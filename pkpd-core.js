...
(function (root) {
  ...
  function meetsTarget(m, target, mic) {
    switch (target.type) {
      ...
      /* ---- Concentration-dependent killing (aminoglycosides) ----
         Efficacy tracks the peak-to-MIC ratio rather than time above
         MIC, so a target of this class is met by a HIGH exposure. */
      case 'cmaxmic': return mic > 0 && (m.fcmax / mic) >= target.threshold;

      /* ---- Trough ceiling (toxicity avoidance) ----
         The inverse sense of every target above: this one is met by a
         LOW exposure, so a regimen can fail it by giving too much or
         dosing too often. Keeping the two senses in one switch is
         deliberate — the whole argument for once-daily aminoglycoside
         dosing is that the peak and the trough are optimised in
         opposite directions. */
      case 'cminceil': return m.cmin <= target.hi + 1e-12;

      /* ---- Composite: every sub-target must be met by the SAME
         simulated subject. Not the product of the marginal
         probabilities — a subject who attains the peak may be the same
         one who breaches the trough. */
      case 'composite':
        for (var i = 0; i < target.all.length; i++) {
          if (!meetsTarget(m, target.all[i], mic)) return false;
        }
        return true;
  ...
  */
  function simulate(cfg) {
    ...
    /* For a composite target, the joint PTA alone hides WHICH constraint a
       regimen fails — and that is the teaching point for aminoglycosides:
       thrice-daily dosing misses the peak while once-daily in renal
       impairment breaches the trough. Report each component separately
       alongside the joint figure. */
    var components = null;
    if (target.type === 'composite') {
      components = target.all.map(function (sub) {
        var k = 0;
        for (var q = 0; q < n; q++) if (meetsTarget(exposures[q], sub, refMic)) k++;
        return { label: sub.label, type: sub.type, pta: (100 * k) / n };
      });
    }
